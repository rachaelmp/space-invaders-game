...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'images/explosion-1_{n}.png') for n in range(6)]
    alien_images = [[pg.image.load(f'images/alien-{m+1}_{n}.png') for n in range(4)] for m
                    in range(4)]
    alien_points = [10, 20, 50, 500]
    ...
    def create_alien(self, row, col):
        ...
        alienindex = row // 2
        if randint(0, 30) > 28:             # randomly include high point aliens
            alienindex = 3
        alien = Alien(game=self.game, sound=self.sound, alien_index=alienindex, ul=(x, y), v=self.v, image_list=images)
    ...
    def set_alien_lasers(self, lasers):
        for alien in self.fleet.sprites():
    ...
    def update(self):
        ...
        if self.length() < 10 and not self.music_changed:
            self.sound.play_fast_bg()
            self.music_changed = True
        if self.length() == 0:
            self.sound.play_twinkle()
            print('WAVE CLEAR!')
            self.music_changed = False
            self.sound.play_bg()
...
class Alien(Sprite):
    def __init__(self, game, image_list, alien_index, sound, start_index=0, ul=(0, 100), v=Vector(1, 0)):
        ...
        self.image = pg.image.load('images/alien-4_0.png')
        self.screen_rect = self.screen.get_rect()
        ...
        self.normal_timer = Timer(AlienFleet.alien_images[alien_index], delay=200, is_loop=True)
    ...
    def update(self, delta_s=Vector(0, 0)):
        ...
        # if randint(0, 10000) <= self.settings.alien_fire_chance and not self.dying:
        # self.lasers.fire()
        self.frames += 1
