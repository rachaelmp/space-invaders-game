...
class Ship(Sprite):
    exploding_images = [pg.image.load(f'images/explosion-2_{n}.png') for n in range(8)]
    firing_images = [pg.image.load(f'images/ship_{n}.png') for n in range(7)]
    images = [pg.image.load(f'images/ship_0.png') for n in range(1)]
    ...
    def __init__(self, game):
        ...
        self.firing_timer = Timer(image_list=Ship.firing_images, delay=20, is_loop=True)
    ...
    def toggle_firing(self):
        ...
        if self.firing and not self.dying:
            self.timer = self.firing_timer
        if not self.firing and not self.dying:
            self.firing_timer.reset()
            self.timer = self.normal_timer

    def hit(self):
        self.timer = self.exploding_timer
        self.sound.play_dies()
        self.dying = True
    ...
    def update(self):
        if self.dying and not self.timer.is_expired():
            self.v = self.v * 0
        ...
        if self.frames % 8 == 0 and self.firing and not self.dying:
            self.lasers.fire()
        self.frames += 1
